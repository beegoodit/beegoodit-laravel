...
Options: `--missing-only` (only generate when the thumbnail file is missing), `--force` (overwrite existing thumbnails), `--dry-run` (report what would be done without writing), `--id=<uuid>` (limit to one feed item).
