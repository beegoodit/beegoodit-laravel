...
class WebPresence
{
    public static function make(): Section
    {
        ...
        $customDomainRecord = Filament::getTenant()->domains()->where('type', 'custom')->first();
        ...
        return Section::make(__('filament-tenancy-domains::domains.web_presence'))
            ...
            ->schema([
                TextInput::make('platform_subdomain')
                    ...
                    ->rules([
                        fn (): \Closure => function (string $attribute, string $value, \Closure $fail) use ($platformDomain): void {
                            $team = Filament::getTenant();
                            $domainPath = $value.'.'.$platformDomain;

                            $exists = Domain::where('domain', $domainPath)
                                ->whereNot(function ($query) use ($team): void {
                                    $query->where('model_type', $team::class)
                ...
                TextInput::make('custom_domain')
                    ...
                    ->unique(Domain::class, 'domain', ignorable: fn () => $customDomainRecord),
                ...
                Section::make(__('filament-tenancy-domains::domains.dns_setup'))
                    ...
                    ->schema([
                        Placeholder::make('dns_instructions')
                            ->hiddenLabel()
                            ->content(fn (): \Illuminate\Support\HtmlString => new HtmlString(sprintf(
                        ...
                        Placeholder::make('verification_status')
                            ...
                            ->content(function (): ?\Illuminate\Support\HtmlString {
                                $record = Filament::getTenant()->domains()->where('type', 'custom')->first();
                        ...
                        Actions::make([
                            Action::make('verify_now')
                                ...
                                ->button()
                                ->color('primary')
                                ->action(function () use ($customDomainRecord): void {
                    ...
                    ->compact()
                    ->visible(fn (): bool => $customDomainRecord && ! $customDomainRecord->is_verified),
            ]);
    }
    ...
    public static function handleSave(array $data): void
    {
        ...
        if ($customDomain = $data['custom_domain'] ?? null) {
            ...
            Filament::getTenant()->domains()->where('type', 'custom')->delete();
        }
    }
}
